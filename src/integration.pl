...
my ( $INPUT_BAM, $INPUT_FASTA, $OUTPUT_FILE, $minSoft, $dist_To_Soft,
    $bedtools, $samtools, $HUMAN_database );
my ( $minRP, $MapQ, $minBQ );
...
if ( defined($INPUT_BAM) ) { $INPUT_BAM = $INPUT_BAM }
else                       { print usage(); die "Where is the BAM file?\n\n" }
if ( defined($INPUT_FASTA) ) { $INPUT_FASTA = $INPUT_FASTA }
else { print usage(); die "Where is the fasta file?\n\n" }
...
if   ( defined($minSoft) ) { $minSoft = $minSoft }
else                       { $minSoft = 5 }
if   ( defined($minRP) ) { $minRP = $minRP }
else                     { $minRP = 2 }
if   ( defined($dist_To_Soft) ) { $dist_To_Soft = $dist_To_Soft }
else                            { $dist_To_Soft = 1000 }
if   ( defined($MapQ) ) { $MapQ = $MapQ }
else                    { $MapQ = 20 }
...
print
    "Usage = HGT.pl -l $minSoft -q $MapQ -d $dist_To_Soft -m $minRP -b $INPUT_BAM -f $INPUT_FASTA -o $OUTNAME \n\n";
...
sub usage {
    ...
    print
        "\t-d\t\tMax distance between soft-clipped segments and discordant read pairs [1000]\n";
...
print
    "Identifying soft-clipped regions that are at least $minSoft bp long \n";
open( FILE, "$random_file_sc" )
    || die "Can't open soft-clipped sam file $random_file_sc\n";
...
while (<FILE>) {
    ...
    if ( $softclip > $minSoft ) {
        ...
        if ( $_[5] =~ /^.*S$/ ) {
            ...
            #RIGHT clip on Minus
            $softBases = substr( $_[9], length( $_[9] ) - $softclip,
                length( $_[9] ) );
            ...
            if ( !$left_corrected ) {
                print "baseCheck($_[2],$end1,left,$softBases)\n";
                next;
            }
...
sub getSeq {
    ...
    if ( $pos < 0 ) {
        print
            "$pos is not a valid position (likely caused by circular MT chromosome)\n";
        return;
    }
...
my $t = "";
print
    "Looking for discordant read pairs without requiring soft-clipping information\n";
use FindBin qw($Bin);
my $path = $Bin;
if ($verbose)   { $v = "-v" }
if ($keep_temp) { $t = "-t" }
...
if ($keep_temp) {
    $command = join( "",
        "perl ",
        $path,
        "/Bam2pair.pl -b $random_file_disc -o $tmp_out -winsize $dist_To_Soft -min $minRP -prefix $random_name -q $MapQ $v $t"
    );
}
else {
    $command = join( "",
        "perl ",
        $path,
        "/Bam2pair.pl -b $random_file_disc -o $tmp_out -winsize $dist_To_Soft -min $minRP -prefix $random_name -q $MapQ $v"
    );
}
if ($verbose) { print "$command\n" }
system("$command");
...
$command = join( "",
    "perl ", $path, "/getregion.pl ",
    $tmp_out, " ", $dist_To_Soft, " > ", $tmp_out_region );
if ($verbose) { print "$command\n" }
system("$command");
#### remove the same genome calls
my $tmp_out_region1 = join( "", $random_name, ".regions.1.out" );
$command = join( "",
    "perl ",         $path, "/filterSame.pl ",
    $tmp_out_region, " ",   $HUMAN_database,
    ".fai",          " > ", $tmp_out_region1 );
if ($verbose) { print "$command\n" }
system("$command");
...
if ( !$keep_temp ) {
    unlink( $tmp_out_region, $tmp_out, $random_file, $tmp_script );
}
