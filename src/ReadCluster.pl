...
my %options = ();
my $results
    = GetOptions( \%options, 'input|i=s', 'output|o=s', 'window|w=s',
    'minClusterSize|m=s', 'help|h' )
    || pod2usage();
...
open( FHD, "<", $options{input} )
    or die "Cound not open file $options{input}\n";
open( INTER, ">", $options{output} . ".inter.sam" )
    or die "Cound not open file $options{output}.inter.sam\n";
