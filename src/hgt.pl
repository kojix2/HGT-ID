...
if ( defined($OUTPUT) ) {
    ...
    if ( -d "$OUTPUT" || -e "$OUTPUT" ) {
        die "Output folder already exist\n";
    }
    elsif ( !-d "$OUTPUT" ) { mkdir $OUTPUT }
}
else { $OUTPUT = getcwd; }
...
if ( -e $soft ) {
    $command = join( "",
        "samtools merge -u -n ", $human_mapping,       "/human.bam ",
        $human_mapping,          "/oneEndMapped.bam ", $human_mapping,
        "/oneEndUnMapped.bam ",  $human_mapping,       "/soft.bam" );
}
else {
    $command = join( "",
        "samtools merge -u -n ",
        $human_mapping, "/human.bam ", $human_mapping, "/oneEndMapped.bam ",
        $human_mapping, "/oneEndUnMapped.bam " );
}
submit($command);
...
$command = join( "",
    ...
    " -Xmx6g -Xms3g -jar ",
    $PICARD,
    "/SamToFastq.jar INPUT=",
    $human_mapping,
    "/human.fix.sort.bam FASTQ=",
    $human_mapping,
    "/read1.fq SECOND_END_FASTQ=",
    $human_mapping,
    "/read2.fq VALIDATION_STRINGENCY=SILENT > ",
    $logs,
    "/1.bam2fastq.log 2>&1" );
submit($command);
...
submit($command);
$command = join( "",
    "samtools merge -u -n ", $viral_mapping,       "/2_virus.bam ",
    $viral_mapping,          "/oneEndMapped.bam ", $viral_mapping,
    "/oneEndUnMapped.bam" );
submit($command);
$command = join( "",
    "samtools sort -@ ",
    $THREADS,       " -n ", $viral_mapping, "/2_virus.bam ",
    $viral_mapping, "/virus.sort" );
submit($command);
$command = join( "",
    "perl ",
    $SCRIPT_DIR,
    "/keepreads.pl ",
    $viral_mapping,
    "/virus.sort.bam | sed '/^\\s*\$/d' | samtools view -u -bS - > ",
    $viral_mapping,
    "/virus.fix.sort.bam" );
submit($command);
...
$command = join( "",
    ...
    "/VIRUS.HUMAN.bam ", $OUTPUT,  "/VIRUS.HUMAN.sort" );
submit($command);
...
$command = join( "",
    "cat ",
    $OUTPUT,
    "/HGT.candidates.txt | perl ",
    $SCRIPT_DIR,
    "/filter.pl  | awk 'NR>1' | awk -v complex=$SEQUENCE_COMPLEXITY '\$NF>=complex' > ",
    $OUTPUT,
...
if ( $chrflag eq "yes" ) {
    $command = join( "",
        "cat ", $OUTPUT, "/regions.bed | grep -w -E '",
        $chrs2keep, "' > ", $OUTPUT, "/regions.2keep.bed" );
}
else {
    $command = join( "",
        "cat ",                                               $OUTPUT,
        "/regions.bed | grep -w -E '",                        $chrs2keep,
        "' | sed -e 's/chrM/chrMT/g' | sed -e 's/chr//g' > ", $OUTPUT,
        "/regions.2keep.bed" );
}
submit($command);
if ( !$debug ) {
    unlink(
        "$OUTPUT/HGT.candidates.txt",
        "$OUTPUT/VIRUS.HUMAN.sam",
        "$OUTPUT/VIRUS.HUMAN.bam",
        "$OUTPUT/VIRUS.HUMAN.sort.bam",
        "$OUTPUT/VIRUS.HUMAN.sort.bam.bai",
        "$OUTPUT/HGT.candidates.flt.txt",
        "$OUTPUT/VIRUS.HUMAN.sort.reads",
        "$OUTPUT/chromosomes2keep.txt",
        "$OUTPUT/VIRUS.HUMAN.flt.sort.reads",
        "$OUTPUT/regions.bed"
    );
}
...
if ( -z "$OUTPUT/regions.2keep.bed" ) {
    ...
    print OUT
        "HumanChr\tHIntegrationPos\tHChr\tHStart\tHPosition\tVirusID\tVStart\tVirusEnd\tDiscordantReads\tSoftclippingReads\n";
    close OUT;
...
$command = join( "",
    "samtools view -b -F 256 -F 1024 -L ",
    $OUTPUT, "/regions.2keep.bed ",
    $BAMFILE, " ", $whichchr, " > ", $OUTPUT, "/human.org.bam" );
...
if ( $chrflag eq "no" ) {
    ...
    $command = join( "",
        ...
        $OUTPUT, "/human.org.bam" );
    submit($command);
}
$command = join( "",
    "samtools view -b -f2 -F 256 -F 1024 -L ",
    $OUTPUT,              "/regions.2keep.bed ",
    $human_mapping_again, "/human.bam > ",
    $OUTPUT,              "/human.again.bam" );
submit($command);
$command = join( "",
    "samtools view -b -f2 -F 256 -F 1024 -L ",
    $OUTPUT,        "/regions.2keep.bed ",
    $viral_mapping, "/virus.bam > ",
    $OUTPUT,        "/virus.org.bam" );
submit($command);
print "merge the BAM file to find the integration point...\n";
$command = join( "",
    "samtools merge -f ", $OUTPUT,                      "/merged.bam ",
    $OUTPUT,              "/VIRUS.HUMAN.flt.sort.bam ", $OUTPUT,
    "/human.org.bam ",    $OUTPUT,                      "/human.again.bam ",
    $OUTPUT,              "/virus.org.bam" );
submit($command);
$command = join( "",
    "samtools sort -@ ", $THREADS,       " ",
    $OUTPUT,             "/merged.bam ", $OUTPUT,
    "/",                 $SAMPLE,        ".forcalling" );
submit($command);
...
if ( !$debug ) {
    unlink(
        "$OUTPUT/merged.bam",    "$OUTPUT/VIRUS.HUMAN.flt.sort.bam",
        "$OUTPUT/human.org.bam", "$OUTPUT/human.again.bam",
        "$OUTPUT/virus.org.bam", "$OUTPUT/VIRUS.HUMAN.flt.sort.bai"
    );
}
...
$command = join( "",
    "mv ",   $OUTPUT,                 "/",
    $SAMPLE, ".forcalling.cyto.bam ", $OUTPUT,
    "/",     $SAMPLE,                 ".forcalling.bam" );
submit($command);
$command
    = join( "", "samtools index ", $OUTPUT, "/", $SAMPLE, ".forcalling.bam" );
submit($command);
...
$command = join( "",
    "perl ",               $SCRIPT_DIR,           "/integration.pl -b ",
    $OUTPUT,               "/",                   $SAMPLE,
    ".forcalling.bam -f ", $VIRUS_HUMAN_database, " -o ",
    $OUTNAME,              " -d ",                $libsize,
    " -m ",                $MINRP,                " -l ",
    $MINSOFT,              " -r ",                $HUMAN_database,
    " -t -v " );
submit($command);
...
$command = join( "",
    "cat ",
    $OUTNAME,
    " | awk '{if(NR==1){print \"#chr\tstart\tend\t\"\$0} else {print \$1\"\\t\"\$2\"\\t\"\$2\"\\t\"\$0}}' > ",
    $OUTNAME,
    ".bed" );
submit($command);
$command = join(
...
$command = join( "",
    ...
    $OUTNAME,           ".gene.txt ", $OUTNAME,
    " | grep -v NA > ", $OUTNAME,     ".annot.txt" );
submit($command);
$command = join( "", "mv ", $OUTNAME, ".annot.txt ", $OUTNAME );
submit($command);

if ( !$debug ) {
    unlink( "$OUTPUT/gene.bed", "$OUTNAME.gene.txt",
        "$OUTPUT/regions.2keep.bed" );
}
if ( !$debug ) {
    rmtree( "$human_mapping", "$human_mapping_again", "$viral_mapping" );
}

#### filter the results if it is both the human pairs

open VIRUSDB, "$VIRUS_database.fai"
    or die "can't open the VIRUS index file\n";
my %virus;
while ( my $l = <VIRUSDB> ) {
    chomp $l;
...
### primer design
$command = join( "",
    "sh ", $SCRIPT_DIR, "/PrimerDesign_HGT_ID.sh ",
    $OUTNAME, " ", $OUTPUT, " ", $CONFIG );
submit($command);
...
mkdir $plot;
$command = join( "",
    "Rscript ", $SCRIPT_DIR, "/circos.Rscript ",
    $OUTNAME, " ", $OUTPUT, " ", $plot, " ", $RLIB );
submit($command);
...
if ( !$debug ) {
    unlink(
        "$viral_mapping/oneEndMapped.bam",
        "$viral_mapping/2_virus.bam",
        "$viral_mapping/oneEndUnMapped.bam",
        "$viral_mapping/virus.bam",
        "$viral_mapping/virus.fix.sort.bam",
        "$viral_mapping/virus.sort.bam"
    );
}
if ( !$debug ) {
    rmtree( "$human_mapping", "$human_mapping_again", "$viral_mapping" );
...
$command = join( "",
    "perl ", $SCRIPT_DIR, "/create_html.pl ", $OUTNAME,
    " ",     $OUTPUT,     "/output.primers ", $OUTPUT,
    " > ",   $OUTPUT,     "/results.html" );
submit($command);
