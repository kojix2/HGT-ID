...
<P><P><P><BR>};
$Integrationdetail
    = qq{<H1><A NAME="Integrationdetail"></A><BR>table description for Integration Site</H1><BR>
...
<P><P><P><BR>};
$Primerdetail
    = qq{<H1><A NAME="Primerdetail"></A><BR>table description for Primer design</H1><BR>
1. Event - event ID<BR>
...
while ( my $l = <FH> ) {
    ...
    for ( $i = 0; $i <= $#a; $i++ ) {
        if ( $. > 1 && $i == 0 && $a[$i] ne "NA" ) {
            $entry
                = qq{<TD ALIGN="LEFT"><a href="http://www.genecards.org/cgi-bin/carddisp.pl?gene=$a[$i]&keywords=$a[$i]" target="_blank">$a[$i]</TD>};
        }
        else {
...
print
    qq{<link rel="stylesheet" href="fancybox/source/jquery.fancybox.css?v=2.0.6" type="text/css" media="screen" />
...
$human_circos
    = qq{<a class="fancybox" rel="HGT-ID"  href="circosplot/human.circos.png" title="HGT-ID"> <img src="circosplot/human.circos.png" width="200"></a>};
print $human_circos;
### VIRAL
opendir DIR, "$ARGV[2]/circosplot"
    or die "cannot open dir $ARGV[2]/circosplot: $!";
my @file = readdir DIR;
print
    "<H1>Circos plot showing Viral Sites with overlayed coverage data</H1>\n";
...
foreach $f (@file) {
    if ( $f ne 'human.circos.png' && $f ne '.' && $f ne '..' ) {
        ...
        $human_circos
            = qq{<a class="fancybox" rel="$name"  href="circosplot/$f" title="$name"> <img src="circosplot/$f" width="200"></a>};
        print $human_circos;
    }
}
close DIR;
